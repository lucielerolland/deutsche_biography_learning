...
def gradient(x, beta, y, l, activation, has_constant=True):
    ...
    if activation == 'sigmoid':
        ...
        direct_gradient = (1 / m) * np.transpose(x).dot(softmax(x.dot(beta)) - y)
...
def gradient_checker(x, y, beta, epsilon, l, has_constant=True):
    ...
    for i in range(len(vec_beta)):
        ...
        Jplus[i] = cost(x, grad_vec_to_mat(thetaplus, beta_shape), y, l, 'softmax')

        thetaminus = vec_beta
        thetaminus[i] = thetaminus[i] - epsilon
        Jminus[i] = cost(x, grad_vec_to_mat(thetaminus, beta_shape), y, l, 'softmax')
